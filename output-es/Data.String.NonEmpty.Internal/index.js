...
const joinWith1 = dictFoldable1 => {
  ...
  return sep => xs => $1.foldl(v => x => {
    if (v.init) { return {init: false, acc: x}; }
    return {init: false, acc: v.acc + (sep + x)};
  })({init: true, acc: ""})(xs).acc;
};
const joinWith = dictFoldable => splice => xs => dictFoldable.foldl(v => x => {
  if (v.init) { return {init: false, acc: x}; }
  return {init: false, acc: v.acc + (splice + x)};
})({init: true, acc: ""})(xs).acc;
const join1With = dictFoldable1 => {
  const $1 = dictFoldable1.Foldable0();
  return splice => xs => $1.foldl(v => x => {
    if (v.init) { return {init: false, acc: x}; }
    return {init: false, acc: v.acc + (splice + x)};
  })({init: true, acc: ""})(xs).acc;
};
